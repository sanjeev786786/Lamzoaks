...
if ( ! class_exists('wp_PayPalAPI') ) {
  ...
  class wp_PayPalAPI {
    ...
    function StartExpressCheckout() {
	  ...
	  global $pricetotal, $listing_id, $price_package, $lp_total_price;
	  ...
	  $enableTax = false;
				...
				$_SESSION['price_packages'] = $price_package;
				if($enableTax==true && !empty($Taxrate)){
					$_SESSION['lptotalPrice'] = $pricetotal;
				}
				else{
					$_SESSION['lptotalPrice'] = $pricetotal;
				}
    ...
    function UpdatePayment($result, $status,$listing_id) {
		if($status=="success"){
				...
				$dbprefix = $wpdb->prefix;
				$price_packages = $_SESSION['price_packages'];
				$lptotalPrice = $_SESSION['lptotalPrice'];
				...
				if( !empty($price_packages) ){
					foreach( $price_packages as $type=>$val ){
						...
						$priceKeyArray[] = $type;
						update_post_meta( $listing_id, $type, 'active' );
					}
				}
				...
				lp_save_campaign_data($adID, $tID, $payment_method, $token, $status, $priceKeyArray, '', $listing_id);

	...
	public function setup_wizard_header() {
		...
		public function envato_setup_introduction() {
			...
			if ( $this->is_possible_upgrade() ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the Easy Setup Assistant! for %s.', 'listingpro' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. ', 'listingpro' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the Easy Setup Assistant!', 'listingpro' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content. It should only take 5 minutes.', 'listingpro' ), wp_get_theme() ); ?><br/><strong>It should only take 5 minutes</strong></p>
				<p><?php esc_html_e( 'No time right now? Skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'listingpro' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'listingpro' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( "Let's Install Required Plugins", 'listingpro' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'listingpro' ); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'listingpro' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress. Click Continue to install', 'listingpro' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue','listingpro' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'listingpro' ); ?></a>
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Let’s Import Dummy Content', 'listingpro' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'listingpro' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( "It's time to import some dummy content for your new site. Click Import Content and once imported, this content can be managed from the WordPress admin dashboard. If you don’t like to import, click Skip this step. ", 'listingpro' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
	  ...
	  $file = get_template_directory().'/include/setup/content/content.xml';
	  if ( !defined('WP_LOAD_IMPORTERS') ) define('WP_LOAD_IMPORTERS', true);
		...
		public function listingpro_theme_options() {
			...
			$file = get_template_directory().'/include/setup/content/themeOptions.json';
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'listingpro' ); ?></h1>

			<p class="lp-redirect-page-pragrahi"><?php esc_html_e( 'Your are redirecting to welcome page.', 'listingpro' ); ?> <img src="<?php echo get_template_directory_uri(); ?>/assets/images/admin/setup-loader.gif" width="" height="" alt="" />
			</p>
